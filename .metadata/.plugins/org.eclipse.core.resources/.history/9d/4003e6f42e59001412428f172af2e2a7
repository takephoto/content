...
public class CCAnimation {
    ...
    public static CCAnimation animationWithFrames( ArrayList<CCSpriteFrame> frames,float delay) {
        return new CCAnimation("ani",delay,frames);
    }
...
//        	frames_.addAll(frames);
        	...
        	for(CCSpriteFrame frame:frames){
        		if(frame!=null){
        			frames_.add(frame);
        		}
        	}
