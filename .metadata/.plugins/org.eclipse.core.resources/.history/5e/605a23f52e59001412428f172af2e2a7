...
import org.cocos2d.LocalFileStore;
import org.cocos2d.ZipUtil;
...
public class CCTMXMapInfo {
	...
	public final static int TMXLayerAttribZlib = 1 << 3;
...
//			InputStream is = CCDirector.sharedDirector().getActivity().getResources().getAssets().open(xmlFilename);
			is = LocalFileStore.instance().getResourceFileInput(xmlFilename);
...
//				Log.v("====TMX====",""+objectGroup.groupName);
				...
				try {
					...
					if(attributes.getValue("height")!=null){
						...
						y = (int) ((mapSize.height * tileSize.height) - y);
					}
...
//			Log.v("====TMX=22===",""+elementName);
			if (elementName.equals("data") && (layerAttribs&TMXLayerAttribBase64) != 0) {
				...
				byte[] buffer = null;
				try {
					buffer = Base64.decode(currentString.toString(),Base64.DONT_GUNZIP);
				} catch (IOException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
				...
				if ((layerAttribs & TMXLayerAttribGzip) != 0) {
					try {
						buffer = ZipUtil.uncompress(buffer);
					} catch (Exception e) {
				...
				else if ((layerAttribs & TMXLayerAttribZlib) != 0) {
					try {
						buffer = ZipUtil.uncompressZlib(buffer);
					} catch (Exception e) {
						ccMacros.CCLOG(LOG_TAG, "cocos2d: TiledMap: inflate data error");
						return;
					}
