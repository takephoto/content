...
//    	typeface = Typeface.DEFAULT;
        ...
        float ascentf = -textPaint.ascent();  // Paint.ascent is negative, so negate it
        float descentf = textPaint.descent();
        
        if(ascentf == 0.0 && descentf == 0.0){
        	ascentf = getSepcAscent(fontSize);
        	descentf = getSepcDescent(fontSize);
        }

//        Log.v("<><><><><><><>","ascent="+textPaint.ascent()+",descent="+textPaint.descent()+",fontSize="+fontSize+",text="+text+",fontname="+fontname);
//        Log.v("<><><><><><><>","ascent1="+textPaint.getFontMetrics().ascent+",descent1="+textPaint.getFontMetrics().descent);
        int ascent = (int) Math.ceil(ascentf);  // Paint.ascent is negative, so negate it
        int descent = (int) Math.ceil(descentf);
...
//        }
    ...
    private final static double[] ascent={
    	8.599991  ,
    	9.459991  ,
    	10.319992 ,
    	11.179993 ,
    ...
    private final static double[] descent={
    	 1.3999939,
    	 1.5399933,
    	 1.6799927,
    	 1.8199921,
    ...
    private static float getSepcAscent(float fontSize){
    	int f = (int)fontSize;
    	int i = f - 10;
    	if(i>14)i=14;
    	return (float)ascent[i];
    }
    private static float getSepcDescent(float fontSize){
    	int f = (int)fontSize;
    	int i = f - 10;
    	if(i>14)i=14;
    	return (float)descent[i];
    }
...
//    	}
        ...
        if(ascent == 0.0 && descent == 0.0){
        	ascent = getSepcAscent(fontSize);
        	descent = getSepcDescent(fontSize);
        }
