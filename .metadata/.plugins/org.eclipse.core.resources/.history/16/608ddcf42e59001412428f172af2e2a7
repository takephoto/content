...
//        scaleX_ = scaleY_ = 1.0f;
    ...
    */
    ...
    //added by yirui 0320
    public CGPoint convertPrevTouchToNodeSpace(MotionEvent event) {
		...
		int pid = action >> MotionEvent.ACTION_POINTER_ID_SHIFT;
        if(Build.VERSION.SDK_INT >= 5) {
        	CCDirector.sharedDirector().convertToGL(Util5.getHistoryX(event, pid), Util5.getHistoryY(event, pid), point);
        } else {
        	CCDirector.sharedDirector().convertToGL(event.getHistoricalX(N), event.getHistoricalY(N), point);
        }
...
//        node._setZOrder(z);
    ...
    */
	...
	public static void ADD_SPRITE(String frameName,CGPoint anchor,CGPoint pos,CCNode parent){
		CCSprite frame = CCSprite.spriteWithSpriteFrameName(frameName); 
		frame.setAnchorPoint(anchor); 
		frame.setPosition(pos); 
		parent.addChild(frame);
	}
	public static void ADD_SPRITE_FILE(String frameName,CGPoint anchor,CGPoint pos,CCNode parent){
		CCSprite frame = CCSprite.sprite(frameName); 
		frame.setAnchorPoint(anchor); 
		frame.setPosition(pos); 
		parent.addChild(frame);
	}
	public static void ADD_LABEL(String text,ccColor3B color,int size,String font,CGPoint anchor,CGPoint pos,CCNode parent){
		CCLabel lbl = CCLabel.makeLabel(text, font, size);
		lbl.setAnchorPoint(anchor); 
		lbl.setPosition(pos); 
		lbl.setColor(color);
		parent.addChild(lbl);
	}
	public static void ADD_MASK(int opacity,CCNode parent){
		CGSize winSize = CCDirector.sharedDirector().winSize(); 
		CCSprite mask = CCSprite.spriteWithSpriteFrameName("mask.png"); 
		mask.setPosition(ccp(winSize.width / 2, winSize.height / 2)); 
		mask.setScale(winSize.width * 2 / mask.getContentSize().width); 
	...
	public static CGPoint ccp(double x,double y){
		return CGPoint.ccp(x, y);
	}
	public static CCNode _makeBg3(String bottom, String left, String top, String center, CGPoint lbpos, CCNode parent)
	{
		return _makeBg3(bottom,left,top,center,lbpos,parent,-1);
	}
	public static CCNode _makeBg3(String bottom, String left, String top, String center, CGPoint lbpos, CCNode parent, int tag)
	{
	    CCSprite sBottom = CCSprite.spriteWithSpriteFrameName(bottom);
		sBottom.setAnchorPoint(CGPoint.ccp(0,0));
		sBottom.setPosition(lbpos);
		parent.addChild(sBottom, 0, tag);
	    
	    CCSprite sBottom2 = CCSprite.spriteWithSpriteFrameName(bottom);
		sBottom2.setAnchorPoint(CGPoint.ccp(0,0));
	    sBottom2.setFlipX(true);
		sBottom2.setPosition(CGPoint.ccp(sBottom.getContentSize().width, 0));
		sBottom.addChild(sBottom2);
	    
		CCSprite sleft = CCSprite.spriteWithSpriteFrameName(left);
		sleft.setAnchorPoint(CGPoint.ccp(0,0));
		sleft.setPosition(CGPoint.ccp(0, sBottom.getContentSize().height ));
		sBottom.addChild(sleft);
	    
		CCSprite sright = CCSprite.spriteWithSpriteFrameName(left);
	    sright.setFlipX(true);
		sright.setAnchorPoint(CGPoint.ccp(0,0));
		sright.setPosition(CGPoint.ccp(sBottom.getContentSize().width * 2 - sright.getContentSize().width, sBottom.getContentSize().height ));
		sBottom.addChild(sright);
	    
		CCSprite stop = CCSprite.spriteWithSpriteFrameName(top);
		stop.setAnchorPoint(CGPoint.ccp(0,0));
		stop.setPosition(CGPoint.ccp(0, sBottom.getContentSize().height + sleft.getContentSize().height -1));
		sBottom.addChild(stop);
	    
	    CCSprite stop2 = CCSprite.spriteWithSpriteFrameName(top);
	    stop2.setFlipX(true);
		stop2.setAnchorPoint(CGPoint.ccp(0,0));
		stop2.setPosition(CGPoint.ccp(stop.getContentSize().width, sBottom.getContentSize().height + sleft.getContentSize().height -1));
		sBottom.addChild(stop2);

	    
		CCSprite scenter = CCSprite.spriteWithSpriteFrameName(center);
		scenter.setAnchorPoint(CGPoint.ccp(0,0));
		scenter.setPosition(CGPoint.ccp(sleft.getContentSize().width - 2, sBottom.getContentSize().height - 2));
		scenter.setScale(2.1f);
		sBottom.addChild(scenter,-1);
	...
	private static final String PACKAGE = CCDirector.theApp.getPackageName();
	private int getResId(String resName){
		return CCDirector.theApp.getResources().getIdentifier(resName,RAW, PACKAGE);
	}
	public void playBackgroundMusic(String resName,boolean loop){
		int resId = getResId(resName);
		if(resId==0)return;
		SoundEngine.sharedEngine().playSound(CCDirector.theApp, resId, loop);
	}
	public void playEffect(String resName){
		int resId = getResId(resName);
		if(resId==0)return;
		SoundEngine.sharedEngine().playEffect(CCDirector.theApp, resId);
	}
	public int playEffect(String resName,boolean loop){
		int resId = getResId(resName);
		if(resId==0)return resId;
		SoundEngine.sharedEngine().playEffect(CCDirector.theApp, resId,loop);
		return resId;
	}
	public void stopEffect(int resId){
		if(resId==0)return;
		SoundEngine.sharedEngine().stopEffect(CCDirector.theApp, resId);
	}
	public void stopEffect(String resName){
		int resId = getResId(resName);
		if(resId==0)return;
		SoundEngine.sharedEngine().stopEffect(CCDirector.theApp, resId);
	}
